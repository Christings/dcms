...
public class CommonDao<T, PK extends Serializable> implements ICommonDao {
	...
	public Integer getRowCount(DetachedCriteria criteria) {
		Integer obj = (Integer) ((Criteria) criteria.setProjection(Projections.rowCount())).uniqueResult();
		return obj;
	}
	...
	public <T> List<T> getPageListByHql(final String hql, Page page) {
		...
		int count = new Integer(countQuery.uniqueResult().toString()).intValue();
	 ...
	 */
	@SuppressWarnings("deprecation")
	public Integer getCountForJdbcInt(String sql, Object[] objs) {
	...
	public HashList getData(String sql) throws DataAccessException {
		logger.info(sql);
		List<Map<String, Object>> list = findForJdbc(sql, new Object[] {});
		return new HashList(list);
	}

	public HashList getData(String sql, Object... args) throws DataAccessException {
		logger.info(sql);
		List<Map<String, Object>> list = findForJdbc(sql, args);
		return new HashList(list);
	}
	...
	public HashList getDataPage(String sql, Page page) {
		...
		try {
			int count = getCountForJdbcInt(sqlCount);
			page.init(count);
			strb.append(sql + " LIMIT " + ((page.getPageNo() - 1) * page.getPageSize()) + "," + page.getPageSize());
			list = getData(strb.toString());
		} finally {
			StringBuilder strb5 = new StringBuilder();
			strb5.append("\r\n");
			strb5.append("  [PreparedStatement SQL:]");
			strb5.append(sqlCount);
			strb5.append("\r\n");
			strb5.append("  [PreparedStatement SQL:]");
			strb5.append(strb.toString());
			logger.info(strb5.toString());
		}
		return new HashList(list);
	...
	public HashList getDataPage(String sql, Object[] args, Page page) {
		...
		List<Map<String, Object>> list = null;
		try {
			int count = getCountForJdbcInt(sqlCount, args);
			page.init(count);
			strb.append(sql + " LIMIT " + ((page.getPageNo() - 1) * page.getPageSize()) + "," + page.getPageSize());
			list = getData(strb.toString(), args);
		} finally {
			StringBuilder strb5 = new StringBuilder();
			strb5.append("\r\n");
			strb5.append("  [PreparedStatement SQL:]");
