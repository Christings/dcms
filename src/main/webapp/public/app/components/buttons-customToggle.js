...
import { Button, ButtonToolbar, Navbar, ButtonGroup, DropdownButton, MenuItem, SplitButton, Dropdown } from 'react-bootstrap'
	...
	render(){
		...
		return (
			<div className={"dropdown-menu"} style = {{ padding: ''}}>
				<input ref={input => this.input = input } type="text" className="form-control" placeholder = "type to filter.." 
					onChange={this.onChange}  value={this.state.value}/>
				<ul className="list-unstyled">{ this.filterChildren() }</ul>
			</div>
		);
	}
}
